...
const TotalDisponibleCtx = createContext(30000000);
...
function Textlink1({ onClick }: { onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
...
function Frame4({ isOn, onToggle, customText }: { isOn: boolean; onToggle: () => void; customText?: string }) {
  return (
    <div className="content-stretch flex gap-[4px] items-start relative shrink-0 w-full">
      <Frame5 customText={customText} />
      <None0Px />
...
function Control({ isOn }: { isOn: boolean }) {
  return (
    <div className={`h-[20px] relative rounded-[9999px] shrink-0 w-[35px] transition-colors duration-200 ${isOn ? 'bg-[#434ce4]' : 'bg-white'}`} data-name="Control">
      <div className={`content-stretch flex items-center ${isOn ? 'justify-end' : 'justify-start'} overflow-clip relative rounded-[inherit] size-full transition-all duration-200`}>
        <Handle isOn={isOn} />
      </div>
      <div aria-hidden="true" className={`absolute border ${isOn ? 'border-[#434ce4]' : 'border-[#8788ab]'} border-solid inset-0 pointer-events-none rounded-[9999px] ${isOn ? 'shadow-[0px_0px_0px_2px_#dee9ff]' : ''}`} />
    </div>
  );
}
...
function Switch({ isOn, onToggle }: { isOn: boolean; onToggle: () => void }) {
  return (
    <div
      onClick={onToggle}
      className="content-stretch flex items-start px-0 py-[2px] relative shrink-0 cursor-pointer"
      data-name="Switch"
    >
      <Control isOn={isOn} />
    </div>
...
function Frame5({ customText }: { customText?: string }) {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start leading-[0] min-h-px min-w-px not-italic relative">
      ...
      <div className="flex flex-col font-['Inter',sans-serif] h-[24px] justify-center relative shrink-0 text-[#282834] text-[14px] w-full" style={{ fontFeatureSettings: "'case'" }}>
...
function Frame6({ isOn, onToggle, customText, onModificarClick }: { isOn: boolean; onToggle: () => void; customText?: string; onModificarClick?: () => void }) {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col gap-[8px] items-start min-h-px min-w-px relative">
      <Frame4 isOn={isOn} onToggle={onToggle} customText={customText} />
      <Textlink1 onClick={onModificarClick} />
    </div>
  );
}

function Programado({ isOn, onToggle, customText, onModificarClick }: { isOn: boolean; onToggle: () => void; customText?: string; onModificarClick?: () => void }) {
  return (
    <div className="bg-white relative rounded-[20px] shrink-0 w-[328px]" data-name="Card">
      <div className="content-stretch flex gap-[16px] items-start overflow-clip p-[16px] relative rounded-[inherit] w-full">
        <Thumbnail />
        <Frame6 isOn={isOn} onToggle={onToggle} customText={customText} onModificarClick={onModificarClick} />
      </div>
    ...
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full">
      <div className="content-stretch flex flex-col gap-[48px] items-start px-[16px] py-0 relative size-full">
        ...
        <div className="shrink-0 w-[328px]">
          <ProgramadoCard customText={customText} isOn={isOn} onToggle={onToggle} onModificarClick={onModificarClick} />
        </div>
        <Cross />
      </div>
    </div>
...
export default function AdminPartiallyConProgramado({
  onAdelantarClick,
  customText,
  onModificarClick,
  isToggleOn = true,
  ...
  return (
    <TotalDisponibleCtx.Provider value={totalDisponible ?? 30000000}>
