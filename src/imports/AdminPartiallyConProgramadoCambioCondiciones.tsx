...
function InfoButton({ onClick }: { onClick?: () => void }) {
  return (
    <div onClick={onClick} className="content-stretch flex items-center relative shrink-0 cursor-pointer" data-name="Info button">
      <Info />
    </div>
  );
}

function Frame3WithInfo({ onInfoClick }: { onInfoClick?: () => void }) {
  return (
...
function CardInfo({ onClick, onInfoClick }: { onClick?: () => void; onInfoClick?: () => void }) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-[328px]" data-name="Card Info">
      <Frame3WithInfo onInfoClick={onInfoClick} />
      <Textlink onClick={onClick} />
    </div>
  );
}

function Frame2({ onClick, onInfoClick }: { onClick?: () => void; onInfoClick?: () => void }) {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full">
      <CardInfo onClick={onClick} onInfoClick={onInfoClick} />
    </div>
  );
}
...
function Frame({ onAdelantarClick, customText, onRevisarCostoClick, onInfoClick }: { onAdelantarClick?: () => void; customText?: string; onRevisarCostoClick?: () => void; onInfoClick?: () => void }) {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full">
      <div className="content-stretch flex flex-col gap-[48px] items-start px-[16px] py-0 relative size-full">
        <Frame2 onClick={onAdelantarClick} onInfoClick={onInfoClick} />
...
export default function AdminPartiallyConProgramadoCambioCondiciones({ 
  onAdelantarClick, 
  customText, 
  onRevisarCostoClick,
}: { 
  ...
  return (
    <div className="bg-white relative rounded-[16px] size-full" data-name="Admin">
      <div className="content-stretch flex flex-col gap-[24px] items-start overflow-clip pb-[24px] pt-0 px-0 relative rounded-[inherit] size-full">
        <Frame1 />
        <Frame onAdelantarClick={onAdelantarClick} customText={customText} onRevisarCostoClick={onRevisarCostoClick} onInfoClick={handleInfoClick} />
