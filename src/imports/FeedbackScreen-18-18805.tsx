...
function Text({ frequencyText }: { frequencyText: string }) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start not-italic relative shrink-0 text-[#282834] w-full" data-name="Text">
      <div className="flex flex-col font-['Inter:Bold',sans-serif] justify-center leading-[28px] relative shrink-0 text-[24px] w-full" style={{ fontFeatureSettings: "'case'" }}>
        <p className="css-4hzbpn mb-0">{`Recibirás tu adelanto `}</p>
        <p className="css-4hzbpn">{frequencyText}</p>
      </div>
      <p className="css-4hzbpn font-['Inter:Regular',sans-serif] leading-[20px] relative shrink-0 text-[16px] w-full" style={{ fontFeatureSettings: "'case'" }}>
...
function Content1({ onCloseClick, frequencyText }: { onCloseClick?: () => void; frequencyText: string }) {
  return (
  ...
  return (
    <div className="bg-white relative rounded-[16px] shadow-[0px_0px_12px_0px_rgba(40,40,52,0.1)] size-full flex flex-col overflow-hidden" data-name="Feedback screen">
      <div className="flex-1 overflow-auto">
        <Content1 onCloseClick={onCloseClick} frequencyText={frequencyText} />
      </div>
